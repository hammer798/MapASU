...
class locations {
    ...
    init(){
        ...
        var currLoc = location(latitude: -1, longitude: -1, locname: names[0], adjPath:"")
        allLocations.append(currLoc)
        //iterate through
        for index in 1..<names.count {
            var loc:location = location(latitude:coords[index-1].0, longitude:coords[index-1].1, locname:names[index], adjPath:adjacentPaths[index-1])
